    ...
    public function update(Request $request, $id)
    {
        ...
        switch ($action) {
            ...
            case 'status':
                ...
                break;
            case 'reset':
                $admin = DB::table('admins')->where('id', $id)->value('email');
                if ($admin === 'admin' && $is_demo) { // 演示状态下无法修改admin的密码
                   return $this->errorWithInfo('演示状态下无法修改超级管理员（admin）的密码', 400);
                }
    ...
    public function modify(Request $request)
    {
     ...
     switch ($action) {
         ...
         case "reset":
             ...
             $pwd = $user->password;
             $email = $user->email;
             if ($email === 'admin' && $is_demo) { // 演示状态下无法修改admin的密码
                return $this->errorWithInfo('演示状态下无法修改超级管理员（admin）的密码', 400);
             }
