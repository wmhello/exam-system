...
{
    use Tool;
    protected  $model = 'App\Models\Question';  // 当前模型
    protected  $fillable = ['question', 'type_id', 'level_id', 'a', 'b', 'c', 'd', 'correct'];  // 当前模型可以修改和新增的字段
    protected  $resource = 'App\Http\Resources\Question'; // 显示个体资源
    protected  $resourceCollection = 'App\Http\Resources\QuestionCollection'; // 显示资源集合
    ...
    protected  function  getListData($pageSize){
        // 当前列表数据  对应于原来的index
        $data = $this->model::orderBy('id', 'desc')->LevelId()->TypeId()->paginate($pageSize);
...
//        2. 根据校验的结果，计算出可以导入的条数，以及错误的内容
    ...
    protected  function UpdateRule($id){
        return [
            "question" => ["required", Rule::unique("questions")->ignore($id)]
        ];
    }
    ...
    protected function  message(){
        return [
            "question.required" => "题目名称不能为空",
            "question.unique" => "题目名称不能重复",
            "type_id:required" => "题目类型不能为空",
            "type_id:exists" => "题目类型必须是在指定的范围内选择",
            "level_id:required" => "题目层次不能为空",
            "level_id:exists" => "题目层次必须是在指定的范围内选择",
        ];
    }
}
