...
{
    use Tool;
    protected  $model = 'App\Models\Test';  // 当前模型
    protected  $fillable = ["name", "start_time", "end_time", "score", "time", "test_rules", "status"];  // 当前模型可以修改和新增的字段
    protected  $resource = 'App\Http\Resources\Test'; // 显示个体资源
    protected  $resourceCollection = 'App\Http\Resources\TestCollection'; // 显示资源集合
...
//        3.数据无误，进一步处理后保存到数据表里面，有的表需要处理，有的不需要
    ...
    {
        $data['test_rules'] = json_encode($data['test_rules']);
        $data['start_time'] = Carbon::createFromTimestamp($data['start_time']/1000);
        $data['end_time'] = Carbon::createFromTimestamp($data['end_time']/1000);
