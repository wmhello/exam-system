...
class QuestionLevelController extends Controller
{
    ...
    use Tool;
    protected  $model = 'App\Models\QuestionLevel';  // 当前模型
    protected  $fillable = ['name', 'desc', 'num'];  // 当前模型可以修改和新增的字段
    protected  $resource = 'App\Http\Resources\QuestionLevel'; // 显示个体资源
    protected  $resourceCollection = 'App\Http\Resources\QuestionLevelCollection'; // 显示资源集合
...
//        2. 根据校验的结果，计算出可以导入的条数，以及错误的内容
    ...
    protected function storeRule(){
      return [
          "name" => "required|unique:question_levels"
      ];
    }

    protected  function UpdateRule($id){
        return [
            "name" => ["required", Rule::unique("question_levels")->ignore($id)]
        ];
    }
    ...
    protected function  message(){
        return [
            "name.required" => "题目层次名称不能为空",
            "name.unique" => "题目层次名称不能重复"
        ];
    }
}
